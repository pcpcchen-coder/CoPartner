...
final class ExecutorService: NSObject, ExecutorXPCProtocol {
    ...
    /// 第 ① 段：固定為 false。第 ④ 段接上真執行時才會變。
    private static let willExecuteActions = false
    /// 第 ② 段補上 code-signing requirement 驗證時才會變 true。
    private static let verifiesCallerSignature = false
