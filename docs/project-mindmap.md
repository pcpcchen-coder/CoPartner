...
| **階段** | **可運作的軟體**，不是骨架。app 在真機上持續觀察並敘事，**而且已經會真的動手**——沙箱內執行命令與操作 UI 都已真機驗過（每個動作仍需人工確認）|
| **步級進度** | **62 / 66 完成（94%）**——原 58 步 backlog，其中 step 53 展開成 53.1–53.7 |
| **里程碑** | M0 ✅・M1 ⬜・M2 ✅・M2.5 ✅・M3 ⬜・M4 ✅・**M5 🔄 執行端 53.1–53.7 全部真機驗過，剩真雲端 SSE**・M6 ⬜ |
| **程式碼** | Swift 8,606 行（100 檔）／測試 6,161 行（72 檔）／**595 個 XCTest**；Python sidecar 254 行 ／ 15 個 pytest |
| **交付節奏** | **50 個 PR 已合併**進 `main`，全部走 CI 三 job（macos-15）：`swift`・`python`・`app` |
  ...
  root((CoPartnerKit))
    ...
    ActionExecutor 19 檔 1789 行
      ...
      風險判定
        ...
        UIActionGate ✅
      沙箱執行
        SandboxPolicy SbplProfileBuilder ✅ 8 項全綠
...
| **M5** 雲端交棒 | LiteLLM + Claude CU + HUD + 沙箱執行 | 53.1–53.7 | 🔄 **執行端 53.1–53.7 全數真機通過** | XPC ✅・sbpl 8 項全綠 ✅・第一次真執行 ✅ 8／20・第一次動電腦 ✅ 9／3・記憶體修復 ✅ |
...
**目前的真實狀態**：整條鏈都通了，而且**兩種執行能力都已真機驗過**——
2026-08-20 第一次真的執行命令（`cat` 讀回檔案，stdout 帶回對得上的 UUID），
2026-09-03 第一次真的動使用者的電腦（HUD 確認後畫面捲動）。
兩次都是用**本地合成提議**走完全相同的路徑（風險分級 → HUD 人工確認 → token → 全部閘門 → 執行端），
不繞過任何閘門，只是把提議的來源從雲端換成本地，讓第一次真執行發生在完全受控的情況下。

**只剩一格是假的**：「Claude computer-use」——真雲端 SSE 來源尚未接上，串流解析鏈是用假來源驗的。
