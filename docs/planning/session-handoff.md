...
1. 按「開始觀察」跑 30 分鐘以上，每隔幾分鐘開一次選單。
   `toggleObserving` 會自動取樣，所以曲線上看得到閒置段與觀察段的交界。
   **看「近期」那個數字**，不是整體。
2. 若近期斜率明顯為正 → 用 Instruments Allocations 抓是誰；
   線性成長指向無上限緩衝，階梯式跳升指向大塊配置（全螢幕截圖）。
3. 告警再次出現時，**當下截一張 Activity Monitor**（按記憶體排序）。
   那張圖能一次分辨上面三種可能，比任何推理都快。
