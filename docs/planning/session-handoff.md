...
1. 啟動後開一次選單記下起始值，**不要**按開始觀察
2. 之後每隔一段時間開一次選單（十幾分鐘一次即可），讓樣本數累積到 5 筆以上
3. 看那一行的「成長 MB/小時」：
   - 明顯為正 → 閒置真的在漲，接著用 Instruments Allocations 抓是誰
   - 接近 0 → 漲的是別的東西（例如開合選單本身），改成**連續開關選單十幾次**再看
4. 再按「開始觀察」跑幾分鐘，比較兩段斜率
