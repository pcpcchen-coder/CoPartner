...
同一個量測，修正前後對照（都跳過開頭 60 秒的建置尖峰）：

| | 觀察中的穩態斜率 | 相鄰兩筆沒動的比例 | 最長持平區段 |
|---|---|---|---|
| 修正前（6 分鐘） | **+151 MB/小時** | 4% | 沒有 |
| 修正後（26 分鐘）| **+7 MB/小時** | 66% | **8.0 分鐘**（停在 202.0 MB，±0.1）|

修正後出現了修正前完全沒有的東西——**長時間一動也不動的平台**：

```
21:28:25–21:36:24 (8.0 分) 停在 202.0 MB
21:40:31–21:47:28 (7.0 分) 停在 165.7 MB
```

修正前是每 15 秒穩定 +0.5 MB、一路往上、沒有任何持平。
**那條無上限的 `AsyncStream<TileEvent>` 就是成因。**
...
停止後 15 分  164.3 MB
停止後 30 分  134.1 MB
停止後 60 分  134.1 MB   ← 不動了
```

30 分鐘落定在 134.1 MB，再等 30 分鐘完全不變。**這是第一次看到真正的落定點。**
（上一輪只觀察約 2 分鐘，落定在 75.5 MB；全新啟動是 26 MB。）

##### 結案與殘留

- ✅ **觀察中的線性成長已修**：+151 → +7 MB/小時。八小時從 +1.2 GB 變成 +56 MB。
- ⚠️ **殘留**：一次 27 分鐘的觀察之後，閒置落定在 134 MB（全新啟動 26 MB）。
  它是**穩定的**——60 分鐘不再變動——所以是「留著不放的工作集」，不是洩漏。
  以絕對值來說離記憶體告警還很遠，暫不處理。
- ⏳ **沒驗到的**：同一次 app 執行裡反覆開關觀察，那個 134 MB 的底線會不會一輪比一輪高。
  由於觀察中的成長已降到 +7 MB/小時，即使有也慢得多，優先度低。

##### 一個順便看到的東西（非記憶體）

日誌裡 `記憶step數` 會**變少**（15→12、37→36→34→32）。`l1Steps` 在縮，
代表它不是單調累積。這不是 bug 也不影響記憶體，但先記一筆——
下次有人拿這個欄位當「累計產生了幾個 step」用的時候會踩到。
