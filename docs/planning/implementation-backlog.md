...
> **60 / 66 完成（91%）**——原 58 步（含 10.5／23.5 共 60 列），其中 step 53 已展開為 53.1–53.7。
> Phase A–G 的 CI 可驗證部分全部完成、三 job 綠；**42 個 PR 已合併進 `main`**。
> 里程碑：M0 ✅・M2 ✅・M2.5 ✅・M4 ✅ ｜ **M5 🔄 執行端 53.1–53.3、53.7 已真機驗過** ｜ M1 ⬜・M3 ⬜・M6 ⬜。
> 剩下 6 項：`23.5`（⏸ 延後優化）、`24`（M1）、`36`（M3）、`53.5`（🔒 第一次真執行的回報待補）、`53.6-C`（🔒 翻開 UI 動作旗標）、`58`（M6）。
> 視覺化全景見 [`docs/project-mindmap.md`](../project-mindmap.md)。最後同步：2026-09-02（`main` @ `a6d5dfd`）。
...
| 53.3 | `sandbox-exec` profile 成對驗證 | M5 | Fable 5 | ✅ 真機（7 項全綠、0 失敗、0 無效）| 53.2 |
| 53.4 | `posix_spawn` 真執行（A 純值層／B service 端）| M5 | Fable 5 | ✅（A CI・B 由 53.5 翻開）| 53.3 |
| 53.5 | 翻開 `willExecuteActions`（單獨一個 PR）| M5 | Opus 4.8 | 🔄 開關已翻開（PR #30）・🔒 **第一次真執行的回報待補** | 53.4 |
| 53.6 | AX／CGEvent UI 動作執行端（A 純值／B 接線／C 翻開）| M5 | Opus 4.8 | 🔄 A ✅・B ✅ 真機乾跑・**C 🔒 待翻開** | 53.5 |
...
#### Step 53.5 — 翻開 `willExecuteActions = true` 🔄（PR #30 已合併 2026-08-20；🔒 真機「第一次真執行」的回報待補）
...
- **DoD**：真機——按「執行測試」→ HUD 出現本地風險判定 → 按執行 →
  稽核出現 `attempt` ＋ `executed`，且 stdout 含測試檔內容
...
- **DoD**：真機——按「UI 測試」→ HUD 出現 → 按執行 → 畫面真的捲動 ＋ 稽核 `executed`
