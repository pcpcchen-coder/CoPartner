...
#### 7.6.3 ⚠️ 規劃缺口：第 ④ 段做完，接手仍然點不了任何東西
...
- [x] 被擋的動作留稽核（I9，PR #17）
- [ ] **`驗 service：通過` 要親眼看到一次**——目前只是「沒報錯」
- [ ] sbpl profile 的正向與負向對照都通過（第 ③ 段）
- [ ] 路徑跳脫（第 ③ 段 (c)）
- [ ] `scripts/xpc-probe.swift` 在**驗簽啟用後**再跑一次——上次那個乾淨的測量是在
      驗簽還沒啟用時做的，現在兩道機制同時生效，結果的意義不同
- [ ] 逾時路徑至少被觸發驗證過一次（可用一個故意不回覆的假 service，或暫時把逾時調成 1 秒）
...
3. 開始第 ③ 段，先從 (c) 路徑跳脫這種純值問題下手（CI 可測、風險低），
   再處理 (a)(b) 的對照測試設計。
